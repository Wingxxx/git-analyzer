...
def calculate_contribution(local_path, since=None, until=None, commit_limit=None):
    """
    计算贡献度
    :param local_path: 本地仓库路径
    :param since: 开始时间（datetime对象）
    :param until: 结束时间（datetime对象）
    :param commit_limit: 限制分析的提交数量
    :return: dict - 贡献度分析结果
    """
    try:
        if not is_git_repo(local_path):
        ...
        doc_extensions = {'.md', '.txt', '.json', '.xml'}
        
        # 预定义关键字列表
        keywords = {'fix', 'bug', 'feature', 'improve', 'optimize'}
        ...
        for commit in repo.iter_commits(**kwargs):
            ...
            # 基于变更大小的评估
            total_changes = author_stats[author]['total_changes']
...
def generate_markdown_report(local_path, output_file="git_analysis_report.md", since=None, until=None, commit_limit=None, contribution_data=None):
    """
    生成Markdown格式的分析报告
    :param local_path: 本地仓库路径
    ...
    :param until: 结束时间（datetime对象）
    :param commit_limit: 限制分析的提交数量
    :param contribution_data: 预计算的贡献度数据，避免重复计算
    ...
    try:
        ...
        if contribution_data is None:
            contribution_data = calculate_contribution(local_path, since, until, commit_limit)
...
def main():
    ...
    # 使用上一级目录作为测试仓库路径
    test_local_path = os.path.dirname(os.getcwd())
    
    print(f"测试本地路径: {test_local_path}")
    print()
    
    # 检测本地路径
    print("1. 检测本地路径:")
    is_repo = is_git_repo(test_local_path)
    print(f"   {test_local_path} 是Git仓库: {is_repo}")
    print()
    
    if is_repo:
        # 更新仓库
        print("2. 更新仓库:")
        update_repo(test_local_path)
        print()
        
        # 分支操作
        print("3. 分支操作:")
        list_branches(test_local_path)
        print()
        ...
        commits = get_commits(test_local_path, since=since_date)
        
        # 显示前5条提交
        print("最近5条提交:")
        for i, commit in enumerate(commits[:5]):
            print(f"  {i+1}. {commit['hash'][:7]} - {commit['author']} - {commit['date']}")
        ...
        if commits:
            print("5. 提交详情:")
            get_commit_details(test_local_path, commits[0]['hash'])
            print()
        
        # 贡献度分析
        print("6. 贡献度分析:")
        calculate_contribution(test_local_path, since=since_date)
        print()
        
        # 生成Markdown报告
        print("7. 生成分析报告:")
        generate_markdown_report(test_local_path, since=since_date)
        print()
    else:
        print("当前目录不是Git仓库，无法进行分析。")
